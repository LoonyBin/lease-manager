...
# Demo data for development. Not production data, and deliberately not
# production-safe.
#
# Rails runs this from `db:seed`, and also from `db:prepare` when that call is
# the one that creates the database. On the Dokku host `db:prepare` is the
# deploy predeploy task (see app.json), so a rebuild of the production database
# would run this file. Refuse: it would fill a live database with invented
# tenants and invoices, and it cannot even load, because factory_bot and faker
# are development/test gems that the production bundle excludes.
if Rails.env.production?
  puts "Skipping seeds: db/seeds.rb is development demo data and never runs in production."
  puts "Restore real data from a backup instead — see DEPLOYMENT.md."
  return
end
...
# Volumes, overridable so a smaller dataset can be seeded (the seed spec uses
# this to keep its run short). The defaults are the full demo dataset.
owner_count = Integer(ENV.fetch("SEED_OWNERS", 3))
tenant_count = Integer(ENV.fetch("SEED_TENANTS", 12))
unleased_property_count = Integer(ENV.fetch("SEED_UNLEASED_PROPERTIES", 3))
properties_per_owner = -> { Integer(ENV.fetch("SEED_PROPERTIES_PER_OWNER") { rand(3..5) }) }

# Invoices generated by an invoice template, as opposed to the security deposit
# invoices a lease raises for itself on creation and termination.
template_invoices = ->(lease) { lease.invoices.covering.where.not(invoice_template_id: nil) }
...
owners = owner_count.times.map do
...
properties = owners.flat_map do |owner|
  properties_per_owner.call.times.map do
...
tenants = tenant_count.times.map do
...
properties_with_leases = properties.sample([properties.size - unleased_property_count, 0].max)
...
#
# Invoices come from the lease's invoice templates, which Lease creates for
# itself on create (InvoiceTemplates::DefaultBuilder, or a copy of the previous
# lease's templates for a renewal). TemplateInvoiceGenerator returns nil for a
# month the template does not bill, and an existing invoice when that month was
# already generated — so only new records are saved.
...
leases.each do |lease_data|
  ...
  templates = lease.invoice_templates.to_a
  puts "  WARNING: lease #{lease.id} (#{lease_type}) has no invoice templates" if templates.empty?
  ...
  while current_date <= end_date
    templates.each do |template|
      invoice = TemplateInvoiceGenerator.new(template, current_date).call
      next if invoice.nil?

      invoice.save! if invoice.new_record?

      # Finalize older invoices (not current month drafts). Invoice's
      # before_save :assign_number gives them their number on the way.
      invoice.update!(status: :finalized) if current_date < Date.current.beginning_of_month && invoice.draft?
  ...
  existing_dates = template_invoices.call(lease).pluck(:date).map(&:beginning_of_month)
...
leases_with_gaps = Lease.all.count do |l|
  ...
  template_invoices.call(l).where(date: first_month..last_month).count < expected_months
end
