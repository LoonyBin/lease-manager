...
# The behavioural successor to the retired route-walk invariant spec. That spec
# had teeth because it compared live routes against a human-curated allowlist; a
# "derived registry equals its own derivation" check would pass by construction.
# This one asserts the gate's *behaviour* over every grantable action.
#
# It sweeps EVERY grantable action (not one sample per controller): a
# conditional `skip_before_action :enforce_token_permissions, only: :x` leaves
# the callback in the chain (so callback introspection can't see the hole) yet
# lets action :x through — only a per-action behavioural sweep catches it.
